...
impl HimmelblauConfig {
    ...
    /// This function attempts to convert a username to a valid UPN.
    ///
    /// Returns `None` when the name cannot represent a directory user — an empty
    /// string, or a known local-only name (systemd dynamic users, GDM greeters,
    /// the pam_unix sentinel, etc.). Callers MUST treat `None` as "user unknown"
    /// and avoid any lookup against Entra: such probes otherwise burn the per-IP
    /// GetCredentialType throttle bucket. See himmelblau-idm/himmelblau#1392.
    ///
    /// Otherwise returns `Some(upn)` — either the input unchanged (already a UPN
    /// or a real local user), or `<name>@<domain>` via cn_name_mapping / the
    /// name_mapping_script.
    pub fn map_name_to_upn(&self, account_id: &str) -> Option<String> {
        self.map_name_to_upn_impl(account_id, "/etc/passwd")
    }

    /// Inner implementation with an injectable passwd path so the local-user
    /// precedence can be unit-tested hermetically (the public wrapper always
    /// uses the real `/etc/passwd`).
    fn map_name_to_upn_impl(&self, account_id: &str, passwd_path: &str) -> Option<String> {
        ...
        // An empty/whitespace name can never be a valid UPN.
        ...
        // Known local-only names that must never be mapped to a UPN. These are
        // probed at boot / greeter-creation by systemd and GDM and would
        // otherwise each cost a GetCredentialType round-trip to Entra.
        ...
        // if a name is supplied, or to a name if the UPN is supplied. An empty
        // output (or a failed run) signals "not a directory user" -> None.
...
mod tests {
    ...
    // Helper: write a throwaway passwd file so map_name_to_upn_impl can be
    // tested without depending on the host's real /etc/passwd.
    ...
    // Minimal passwd fixture with no AD-shaped names, used by tests that need
    // the input to fall through the local-user check.
    ...
    fn test_map_name_to_upn_local_user() {
        ...
        .unwrap();
        // localuser is present in the fixture passwd -> resolved locally as-is.
    ...
    fn test_map_name_to_upn_local_user_wins_over_ignore_list() {
        // A name that's in the ignore list AND in /etc/passwd must resolve
        // locally (Some), not be rejected — the local-user check has priority.
        let config = HimmelblauConfig::new(Some(&create_temp_config(
            r#"
        [global]
        domains = example.com
    ...
    fn test_map_name_to_upn_ignored_names_are_none() {
        ...
        .unwrap();
        // Fixture has none of the ignored names, so the ignore list fires.
        let passwd = create_temp_passwd(PASSWD_MINIMAL);
        for name in [
            // exact
            "gdm",
